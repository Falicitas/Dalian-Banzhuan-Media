...
public class Upload_frame extends JDialog {
    ...
    public void registerSetting() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (passwordAssure.equals(password)) {
                    ...
                    if (state == 1) {
                        ...
                        isRegister.add(new JLabel("注册成功", SwingConstants.CENTER));
                        ...
                        JDialog isRegister = new JDialog();
                        isRegister.setSize(200, 100);
                        isRegister.add(new JLabel("注册失败（用户名已经存在）", SwingConstants.CENTER));
                        isRegister.setModal(true);
                        isRegister.setLocationRelativeTo(null);
                        isRegister.setVisible(true);
                    ...
                    JDialog isRegister = new JDialog();
                    isRegister.setSize(200, 100);
                    isRegister.add(new JLabel("两次输入密码不一致",SwingConstants.CENTER));
                    isRegister.setModal(true);
                    isRegister.setLocationRelativeTo(null);
                    isRegister.setVisible(true);
        ...
        InputMap imap = registerPasswordAssure.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW);
        imap.put(KeyStroke.getKeyStroke("ENTER"), "register");
        registerPasswordAssure.getActionMap().put("register", action);
...
//        autoLogin.addActionListener();
    ...
    public void loginSetting() {     //登陆按钮设置
        ...
        InputMap imap = password.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW);
        imap.put(KeyStroke.getKeyStroke("ENTER"), "login");
        password.getActionMap().put("login", action);
